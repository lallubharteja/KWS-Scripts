...
if [ ! -f $lang/L_align.fst ]; then
  ...
  local_kws/make_L_align.sh $lang/local/ $lang $lang 2>&1 | tee $dir/log/L_align.log
fi

$cmd $dir/log/align_to_words.log \
  ali-to-phones $dir/final.mdl "ark:gunzip -c $dir/ali.*.gz|" ark,t:- \| \
  phones-to-prons $lang/L_align.fst $wbegin $wend ark:- "ark,s:utils/sym2int.pl -f 2- --map-oov '$oov' $lang/words.txt <$trans_dir/text|" ark,t:- \| \
...
echo "$0: writing rttm."
[ ! -x local_kws/txt_to_rttm.pl ] && \
  echo "Not creating rttm because local/txt2rttm.pl does not exist or not executable." && exit 1;

#local_kws/txt_to_rttm.pl --symtab=$lang/words.txt --segment=$data/segments $dir/align.txt $dir/rttm 2>$dir/log/rttm.log
local_kws/txt_to_rttm.pl --symtab=$lang/words.txt $dir/align.txt $dir/rttm 2>$dir/log/rttm.log
